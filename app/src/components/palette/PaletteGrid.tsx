      ...
      <div className="flex flex-wrap justify-end items-center gap-1 px-4 lg:pr-0 mb-3">
        <div className="flex items-center gap-3 rounded-md bg-surface-raised px-3 py-1.5">
          <label className="flex items-center gap-1 cursor-pointer">
            <span className="text-xs font-mono text-text-tertiary">swatches</span>
            <select
              value={swatchCount}
              onChange={(e) => setSwatchCount(Number(e.target.value))}
              className="text-xs font-mono text-text-subtle bg-transparent border-none outline-none cursor-pointer"
            >
              {STEP_OPTIONS.map((n) => (
          ...
          <label className="flex items-center gap-1 cursor-pointer">
            <span className="text-xs font-mono text-text-tertiary">layout</span>
            <select
              value={layout}
